...
add_action( 'wpcf7_init', 'wpcf7_add_form_tag_wpx_number' );

function wpcf7_add_form_tag_wpx_number() {
	wpcf7_add_form_tag( array( 'wpx_number', 'wpx_number*', 'wpx_range', 'wpx_range*' ),
		'wpcf7_wpx_number_form_tag_handler', array( 'name-attr' => true ) );
}

function wpcf7_wpx_number_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
...
add_filter( 'wpcf7_validate_wpx_number', 'wpcf7_wpx_number_validation_filter', 10, 2 );
add_filter( 'wpcf7_validate_wpx_number*', 'wpcf7_wpx_number_validation_filter', 10, 2 );
add_filter( 'wpcf7_validate_wpx_range', 'wpcf7_wpx_number_validation_filter', 10, 2 );
add_filter( 'wpcf7_validate_wpx_range*', 'wpcf7_wpx_number_validation_filter', 10, 2 );

function wpcf7_wpx_number_validation_filter( $result, $tag ) {
	$name = $tag->name;
...
add_filter( 'wpcf7_messages', 'wpcf7_wpx_number_messages' );

function wpcf7_wpx_number_messages( $messages ) {
...
add_action( 'wpcf7_admin_init', 'wpcf7_add_tag_generator_wpx_number', 19 );

function wpcf7_add_tag_generator_wpx_number() {
	$tag_generator = WPCF7_TagGenerator::get_instance();
	$tag_generator->add( 'wpx_number', __( 'wpx_number', 'contact-form-7' ),
		'wpcf7_tag_generator_wpx_number' );
}


function wpcf7_tag_generator_wpx_number( $contact_form, $args = '' ) {
    ...
    <div class="control-box">
        <fieldset>
            ...
            <table class="form-table">
                ...
                <tr>
                    ...
                    <td>
                        <fieldset>
                            ...
                            <select name="tagtype">
                                <option value="wpx_number"
                                        selected="selected"><?php echo esc_html( __( 'Spinbox', 'contact-form-7' ) ); ?></option>
                                <option value="wpx_range"><?php echo esc_html( __( 'Slider', 'contact-form-7' ) ); ?></option>
                            </select>
                            <br/>
				...
				<?php //echo esc_attr( $args['content'] . '-values' ); ?><!--">-->
				<?php //echo esc_html( __( 'Default value', 'contact-form-7' ) ); ?><!--</label></th>-->
                <!--					<td><input type="text" name="values" class="oneline" id="-->
				<?php //echo esc_attr( $args['content'] . '-values' ); ?><!--" /><br />-->
