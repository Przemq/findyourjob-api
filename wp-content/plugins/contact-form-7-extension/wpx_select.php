<?php
/**
 ** A base module for [wpx_select] and [wpx_select*]
 **/

/* form_tag handler */

add_action( 'wpcf7_init', 'wpcf7_add_form_tag_wpx_select' );

function wpcf7_add_form_tag_wpx_select() {
	wpcf7_add_form_tag( array( 'wpx_select', 'wpx_select*' ),
		'wpcf7_wpx_select_form_tag_handler',
		array(
			'name-attr'             => true,
			'wpx_selectable-values' => true,
		)
	);
}

function wpcf7_wpx_select_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
	...
	$form_id = '';
	if ( ! empty( $tag->get_option( 'form_id' ) ) ) {
		$form_id = $tag->get_option( 'form_id' )[0];
	};
	$defaults = array_unique( $defaults );
	$shifted = false;

	if ( $include_blank || empty( $values ) ) {
		array_unshift( $labels, 'Select your option' );
	...
	foreach ( $values as $key => $value ) {
		$wpx_selected = false;
		$sessionVal   = $form_id . "_" . $tag->name;
		if ( $hangover ) {
			if ( $multiple ) {
				$wpx_selected = in_array( esc_sql( $value ), (array) $hangover );
			} else {
				$wpx_selected = ( $hangover == esc_sql( $value ) );
			}
		} else {
			if ( isset( $_SESSION[ $sessionVal ] ) && $_SESSION[ $sessionVal ] === $value ) {
				$wpx_selected = true;
			}
		...
		$item_atts = array(
			'value'    => $value,
			'selected' => $wpx_selected ? 'selected' : '',
		);
...
add_filter( 'wpcf7_validate_wpx_select', 'wpcf7_wpx_select_validation_filter', 10, 2 );
add_filter( 'wpcf7_validate_wpx_select*', 'wpcf7_wpx_select_validation_filter', 10, 2 );

function wpcf7_wpx_select_validation_filter( $result, $tag ) {
	$name = $tag->name;
...
add_action( 'wpcf7_admin_init', 'wpcf7_add_tag_generator_wpx_menu', 25 );

function wpcf7_add_tag_generator_wpx_menu() {
	$tag_generator = WPCF7_TagGenerator::get_instance();
	$tag_generator->add( 'wpx_menu', __( 'wpx-drop-down menu', 'contact-form-7' ),
		'wpcf7_tag_generator_wpx_menu_select' );
}

function wpcf7_tag_generator_wpx_menu_select( $contact_form, $args = '' ) {
                    ...
                    <td>
                        <fieldset>
                            ...
                            <label><input type="checkbox" name="multiple"
                                          class="option"/> <?php echo esc_html( __( 'Allow multiple wpx_selections', 'contact-form-7' ) ); ?>
                            </label><br/>
    ...
    <div class="insert-box">
        <input type="text" name="wpx_select" class="tag code" readonly="readonly" onfocus="this.wpx_select()"/>
